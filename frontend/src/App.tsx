    ...
    <main className="content">
      ...
      {!loading && data && <>
        ...
        <section className="workspace">
          ...
          <aside className="inspector panel">{selected ? <NodeInspector node={selected} incoming={selectedNeighbors.incoming} outgoing={selectedNeighbors.outgoing} onClose={() => setSelectedId(undefined)} /> : <div className="inspector-empty"><div className="crosshair">⊹</div><h2>Выберите узел</h2><p>Нажмите на точку графа, чтобы увидеть роль, evidence и денежные потоки.</p><div className="tip"><span>TIP</span> Используйте поиск по GID, если нужен конкретный клиент.</div></div>}</aside>
        </section>
