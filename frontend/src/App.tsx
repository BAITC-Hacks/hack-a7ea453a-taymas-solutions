...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {!loading && error && <div className="state-card error-state"><div className="state-icon">!</div><h2>Выгрузки не найдены</h2><p>{error}. Запустите Python-пайплайн, затем обновите страницу.</p><code>python -m money_graph --data data --out out</code><button className="secondary-button" onClick={() => void fetchData('/fixtures')}>Открыть демо-набор</button></div>}
      {!loading && data && <>
        ...
        <section className="workspace">
          ...
          <aside className="inspector panel investigation-rail">
            ...
            <div className="investigation-content inspector-content" role="tabpanel" id="node-view" aria-labelledby="node-tab" hidden={railTab !== 'node'}>{selected ? <NodeInspector node={selected} incoming={selectedNeighbors.incoming} outgoing={selectedNeighbors.outgoing} onClose={() => setSelectedId(undefined)} /> : <div className="inspector-empty"><div className="crosshair">⊹</div><h2>Выберите узел</h2><p>Нажмите на точку графа, чтобы увидеть роль, evidence и денежные потоки.</p><div className="tip"><span>TIP</span> Используйте поиск по GID, если нужен конкретный клиент.</div></div>}</div></aside>
        </section>
