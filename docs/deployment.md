...
пайплайном и только для чтения nginx. Секреты и NVIDIA API tokens для этого
режима не нужны и в контейнеры не передаются.
...
После успешного завершения `pipeline` Compose запускает `ui`. Откройте
