...
Формула — взвешенная сумма шести компонент, каждая в [0, 1]:
  ...
  flow     — транзит или оседание: ушли ли полученные деньги дальше
             ...
             если отправлено больше 1.2 × видимого входа, не оценивается
...
# отдал больше INCOMPLETE_INFLOW_RATIO × видимого входа — вход неполный; обязано совпадать
# с money_graph.config.EXTERNAL_INFLOW_RATIO, иначе роль и priority_why разойдутся
INCOMPLETE_INFLOW_RATIO = 1.2
...
def _features(edges: pd.DataFrame, nodes: pd.DataFrame,
    ...
    # входящих не попала в выгрузку (как у seed). Коридор до 1.2 — тот же, что у транзита
    # в ролях (pass_through 0.8–1.2): в его пределах out/in ещё интерпретируется.
    f["unseen_in_kzt"] = np.where(~f.is_seed & ~f.truncated
                                  & (f.out_kzt > INCOMPLETE_INFLOW_RATIO * f.in_kzt),
...
def _phrase(k: str, r) -> str:
    ...
    if k == "flow":
        ...
        if r.out_kzt > r.in_kzt:
            return f"переслал дальше всё полученное ({r.out_kzt / r.in_kzt:.0%})"
        if forwarded == 1.0:
