...
def _coerce(df: pd.DataFrame, table: str, rep: InputReport) -> pd.DataFrame:
    ...
    for col, kind in schema.items():
        ...
        if n_null:
            rep.errors.append(f"{table}.{col}: {n_null} пустых значений")
            continue
        if kind == "int":
            if not pd.api.types.is_integer_dtype(s):
                num = pd.to_numeric(s, errors="coerce")
                if num.isna().any() or (num % 1 != 0).any():
            ...
            num = pd.to_numeric(s, errors="coerce")
            if num.isna().any():
                rep.errors.append(f"{table}.{col}: ожидались числа, тип {s.dtype}")
                continue
...
def _check_contract(edges, nodes, tx, rep: InputReport):
    ...
    sum_diff = int(((both.sum_kzt - both.tx_sum).abs() > SUM_TOLERANCE_KZT).sum())
