...
Нужен Python 3.11 или новее. Это официальный runtime проекта: `networkx==3.6.1` не устанавливается на Python 3.9. Команды выполняются из корня репозитория:
...
## Экран просмотра графа

После того как pipeline создал `out/nodes_roles.csv`, запустите локальный экран:

```bash
streamlit run app.py
```

Экран поддерживает поиск по `gid`, фильтры по роли, кластеру, глубине и seed, режим просмотра соседей узла или верхних приоритетов, а также переключение цвета узлов между ролями и кластерами. Стрелки на графе показывают направление `src → dst`.
