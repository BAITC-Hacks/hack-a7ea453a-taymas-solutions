...
- **Обработка и проверка данных.** Загрузка трёх Parquet-файлов, проверка полей, типов, идентификаторов и согласованности сумм и количества операций между рёбрами и транзакциями.
...
1. Аналитик размещает `nodes.parquet`, `edges.parquet` и `transactions.parquet` в `data/`.
...
4. Результаты записываются в `out/`. Веб-интерфейс читает эти файлы и открывает окружение первого клиента из топ-листа.
...
```mermaid
flowchart TD
    DATA["data/*.parquet"] --> PIPE["money_graph: проверка, граф, признаки, роли"]
    PIPE --> ANALYTICS["analytics: кластеры, приоритет, топ-лист"]
    ANALYTICS --> OUT["out/*.csv и отчёты"]
    OUT --> UI["React + Cytoscape.js"]
...
| `agent_orchestrator/` | Маршрутизация вопроса, ограниченный план вызовов, сборка ответа, HTTP API и адаптер NVIDIA |
...
Браузер получает CSV через Vite при разработке или nginx в Docker. Copilot держит собственный снимок тех же CSV в памяти и не изменяет расчёты. После пересчёта `out/` нужно перезапустить Copilot и обновить страницу. При недоступности API граф, фильтры и карточки продолжают работать.
...
Откройте терминал в корне клонированного или распакованного проекта. Датасет организаторов распространяется отдельно: `data/` и результаты `out/` исключены из Git. Разместите файлы так:
...
Для запуска без Copilot предусмотрены `-NoCopilot` в Windows и `--no-copilot` в Bash. Vite проксирует API на порт `8765`, поэтому для обычного демо используйте этот порт.

### Ручной запуск отдельных компонентов

В первом терминале из корня проекта создайте окружение, установите зависимости, выполните расчёт и запустите API.
...
После подготовки `data/`, из корня проекта в Bash:

```bash
bash scripts/docker-compose.sh up --build -d
```

В Windows / PowerShell с Docker Desktop:
...
Откройте [http://127.0.0.1:8501](http://127.0.0.1:8501). Compose сначала выполняет пайплайн с `--check-repro`, затем запускает Copilot и nginx. API доступен через `/api/copilot` на том же адресе; отдельный порт API наружу не публикуется. Для остановки выполните `docker compose down` из корня проекта.

На macOS/Linux скрипт согласует UID/GID контейнеров с текущим пользователем для записи в `out/`. Настройка портов, каталогов и дополнительные проверки описаны в [docs/deployment.md](docs/deployment.md).
...
1. **Запустите полный сценарий** с датасетом и флагом проверки воспроизводимости. Убедитесь, что в `out/` появились четыре CSV и отчёты; в `run_report.json` поле `reproducibility.identical` должно быть `true`.
...
3. **Выберите первого клиента в очереди проверки.** Откройте карточку, изучите роль, приоритет, входящие и исходящие суммы. Найдите тот же GID в `out/top_nodes.csv` и `out/nodes_roles.csv`: роль и приоритет должны совпасть с интерфейсом. Длинные GID сравнивайте как строки.
...
Браузерные тесты требуют работающих API, интерфейса и реальных `out/`. Установите Chromium командой `npx playwright install chromium`, затем из `frontend/` укажите адрес запущенного UI.
...
**Локальный API.** `GET /api/copilot/status` сообщает готовность сервиса и наличие настройки NVIDIA. `POST /api/copilot/answer` принимает вопрос, выбранные GID, глубину обхода и флаг использования AI. Источником фактов служат готовые CSV, доступные инструментам только для чтения.
...
- **Локальная работа со снимком.** Нет потокового обновления банковских данных; загрузка Parquet выполняется через каталог и CLI. После пересчёта требуется перезапустить API и обновить страницу.
