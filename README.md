...
Нужны Python 3.11+ и Node.js 20.19+ или 22.12+. Проверены Python 3.11 и 3.13;
команды выполняются из корня репозитория.

Одна команда для локального демо после распаковки `data/`:
...
Скрипт создаёт локальное Python-окружение при необходимости, устанавливает
зависимости, прогоняет пайплайн от `data/*.parquet` до `out/*.csv`, запускает
локальный Copilot API, поднимает Vite UI и открывает его в браузере. Для строгой проверки
воспроизводимости добавьте `-CheckRepro` в Windows или `--check-repro` в
macOS/Linux.

Ручной запуск только пайплайна:
...
Для локального Docker-запуска пайплайна и React-интерфейса см. [инструкцию по деплою](docs/deployment.md): `./scripts/docker-compose.sh up --build` поднимает UI на `http://127.0.0.1:8501` и монтирует `data/`/`out/` с правами текущего пользователя.
...
| Время | `fast_out_share, median_lag_days, sync_payers_max, max_tx_per_day, active_days` | близость дат видимого входа и оттока (≤2 дня), синхронные поступления, всплески |
...
`fast_out_share` — доля наблюдаемого оттока в окне 0–2 календарных дня от
последнего видимого поступления, независимо от суммы этого поступления.
Например, вход 995 000 KZT 1 июля, ещё 5 000 KZT 30 июля и выход 1 000 000 KZT
31 июля дают 100%. Это близость дат, а не прослеженный транзит миллиона свежих
средств: суммы не сопоставляются, порядок внутри дня неизвестен, входящие
извне выборки и часть переводов не видны. Роль `transit` остаётся гипотезой
по совокупности признаков. [Формула и ограничения](docs/features.md#временная-близость-входа-и-оттока-pan-63).
...
| `T2` | transit | не seed, `pass_through` 0.5–1.2 и ≥80% оттока в окне 0–2 дня от видимого входа; связь сумм не установлена | 20 |
